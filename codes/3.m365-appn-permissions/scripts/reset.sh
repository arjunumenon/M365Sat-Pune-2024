...
m365 entra app permission add --appId "$appId" --delegatedPermissions https://graph.microsoft.com/Group.ReadWrite.All --grantAdminConsent

# remove the Tasks.Read permission
echo "Removing the Tasks.Read permission..."
m365 entra app permission remove --appId "$appId" --delegatedPermissions https://graph.microsoft.com/Tasks.Read --force
